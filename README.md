...
Это крутая программа обучения. Вам повезло тут оказаться. С карьерной точки зрения, стратегически, данная программа - очень выгодное вложение времени.
...
пройти ревью ментора, дать доступ и кинуть на проверку одному из пользователей: `svetlanasadovnikova` (Светлана Беляева), `K1ola` (Екатерина Кириллова), `TonyBlock` (Антон Чумаков) и `zorinarsenij` (Арсений Зорин), `SergeyGritsay` (Сергей Грицай). Лучше писать преподавателю в лс, что ментор выдал вам аппрув, прикрепив ссылку на ПР.
